...
class Model(nn.Module):
    ...
    #     # feature = ReverseLayerF.apply(self._forward_feature(x,profile,visualize),-1)
    #     # domain_pred = self.classify(feature)
    #     return self._forward_once(x,profile,visualize),self._forward_feature(x,profile,visualize)
    ...
    def _forward_feature(self, x, profile=False, visualize=False):
        ...
        l = [17,20,23]
        for i in l:
            c = self.classify[l.index(i)]
            xx = c((ReverseLayerF.apply(y[i], -1)))
            pred.append(xx)
        return pred
