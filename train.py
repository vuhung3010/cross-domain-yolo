...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    source_domain_label = torch.zeros(batch_size*3,1).cuda()
    target_domain_label = torch.ones(batch_size*3,1).cuda()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    t_imgs = nn.functional.interpolate(imgs, size=ns, mode='bilinear', align_corners=False)

            # Forward
            with amp.autocast(enabled=cuda):
                pred = model(imgs)
                # pred,source_pred_label = model(imgs)  # forward
                ...
                # print('pred is ', source_pred_label.shape)
                ...
                # loss, loss_items = compute_loss_ota(pred, targets.to(device), imgs) if (compute_loss_ota is not None )else compute_loss(pred, targets.to(device)) # loss scaled by batch_siz
                    ...
                    # loss = loss_ori + loss_source + loss_target
                    loss = loss_ori
                if RANK != -1:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT / 'configs/yolov5-standard/yolov5s.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/domain/tank.yaml', help='dataset.yaml path')
