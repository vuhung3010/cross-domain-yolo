...
Usage:
    $ python path/to/val_GRL.py --weights yolov5s.pt --data coco128.yaml --img 640

Usage - formats:
    $ python path/to/val_GRL.py --weights yolov5s.pt                 # PyTorch
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        if opt.task == 'speed':  # speed benchmarks
            # python val_GRL.py --task speed --data coco.yaml --batch 1 --weights yolov5n.pt yolov5s.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            for opt.weights in weights:
                run(**vars(opt), plots=False)

        elif opt.task == 'study':  # speed vs mAP benchmarks
            # python val_GRL.py --task study --data coco.yaml --iou 0.7 --weights yolov5n.pt yolov5s.pt...
            for opt.weights in weights:
